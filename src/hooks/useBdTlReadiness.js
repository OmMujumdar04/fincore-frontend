...
export default function useBdTlReadiness(entity = "bd") {
    ...
    async function fetchData() {
      ...
      try {
        const [fullRes, summaryRes] = await Promise.all([
          fetch(`${API_BASE}/api/${entity}-readiness`),
          fetch(`${API_BASE}/api/${entity}-readiness/summary`),
        ]);

        if (!fullRes.ok || !summaryRes.ok) {
          throw new Error(`Failed to fetch ${entity} readiness data`);
        }
