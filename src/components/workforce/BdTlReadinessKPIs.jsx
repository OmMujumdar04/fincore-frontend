export default function BdTlReadinessKPIs({ data, summary, computedAt, loading, label }) {
  if (loading) {
  ...
  const formattedTime = computedAt ? new Date(computedAt).toLocaleString() : "—";

  return (
    <div className="space-y-3">
      <p className="text-sm text-gray-400">Data as of {formattedTime}</p>
      <div className="grid md:grid-cols-5 gap-4">
        <div className="bg-white rounded-2xl shadow-md p-6">
          <p className="text-gray-500 text-sm">Total {label}s</p>
